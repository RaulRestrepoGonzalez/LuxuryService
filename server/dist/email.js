...
async function initTransporter() {
    if (CFG.host) {
        ...
        try {
            ...
            console.warn(`[EMAIL] ❌ No se pudo crear transporte directo: ${err.message}`);
...
async function guardarEmailPendiente(data) {
    try {
        ...
        console.error('[EMAIL] Error guardando email pendiente:', err.message);
    }
}
// ── Retry logic ──
async function enviarConReintento(sendFn, params, maxAttempts = 3) {
...
<p style="color: #fff; margin: 0; font-size: 0.85rem; line-height: 1.5;">📍 Recuerda llegar <strong>10 minutos antes</strong> para la revisión inicial. El valor final puede variar según el <strong>estado y tipo de tu vehículo</strong>.</p>
    ...
    return enviarConReintento(() => transporter.sendMail({
        ...
        tipo: 'confirmacion', datos: params,
        html, text,
    });
}
...
export function iniciarColaPendientes() {
    if (workerTimer)
        return;
    ...
    workerTimer = setInterval(async () => {
        try {
            ...
            console.error('[EMAIL] Error en cola de pendientes:', err.message);
        }
    }, CFG.retryIntervalMs);
}
export function detenerColaPendientes() {
    if (workerTimer) {
...
export async function reenviarEmailsPendientes() {
    if (!transporter) {
    ...
    for (const email of pendientes) {
        try {
            ...
            const update = {
                $inc: { intentos: 1 },
                $set: { ultimoError: err.message, proximoIntento: nextRetry },
            };
            if (intentos >= CFG.maxRetries) {
                update.$set['estado'] = 'abandonado';
                console.error(`[EMAIL] 🚫 Abandonado tras ${intentos} intentos: ${email.asunto} -> ${email.to}`);
            }
            await db.collection('pending_emails').updateOne({ _id: email._id }, update);
        }
    }
...
export async function verificarConfiguracion() {
    ...
    if (CFG.dkimDomain)
        console.log(`║  DKIM: ${CFG.dkimDomain.padEnd(48)}║`);
    console.log('║                                                              ║');
