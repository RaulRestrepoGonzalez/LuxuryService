...
export function isFoodProduct(name, supplier) {
    const upperName = name.toUpperCase();
    const words = tokenize(name);
    // Never flag known auto parts
    for (const w of words) {
        if (AUTO_SAFE_WORDS.has(w))
            return false;
    }
    if (upperName.includes('GUARDABARRO'))
        return false;
    if (upperName.includes('PINTURA'))
        return false;
    // Check supplier
    if (supplier && FOOD_SUPPLIERS.has(supplier.toUpperCase()))
        return true;
    // Check individual keywords
    for (const w of words) {
        if (FOOD_KEYWORDS.has(w) || DRINK_KEYWORDS.has(w))
            return true;
    }
    return false;
}
