...
const HORARIO_LABELS = { '10:00': '10:00 a.m.', '14:00': '2:00 p.m.' };
app.use(cors());
...
app.post('/api/auth/client-access', async (req, res) => {
    try {
        ...
        if (emailNorm === ADMIN_EMAIL)
            return res.status(400).json({ error: 'Los administradores deben usar contraseña' });
        ...
        if (!user)
            return res.status(404).json({ error: 'Regístrate primero con tu correo' });
        if (!user.acepta_terminos || !user.consentimiento_datos)
            return res.status(403).json({ error: 'Aceptación de términos pendiente' });
...
app.post('/api/auth/client-register', async (req, res) => {
    try {
        ...
        if (!nombre?.trim() || !emailNorm)
            return res.status(400).json({ error: 'Nombre y correo requeridos' });
        if (!aceptaTerminos || !consentimientoDatos)
            return res.status(400).json({ error: 'Debe aceptar términos y autorizar datos' });
        if (versionTerminos !== TERMINOS_VERSION || versionPolitica !== POLITICA_VERSION)
            return res.status(400).json({ error: 'Versión de documentos desactualizada' });
        if (emailNorm === ADMIN_EMAIL)
            return res.status(400).json({ error: 'El correo del administrador no puede registrarse como cliente' });
        const exists = await getDb().collection('usuarios').findOne({ email: emailNorm });
        if (exists)
            return res.status(400).json({ error: 'El correo ya está registrado. Usa Acceder.' });
...
app.post('/api/auth/register', async (req, res) => {
    ...
    if (!aceptaTerminos || !consentimientoDatos)
        return res.status(400).json({ error: 'Debe aceptar términos y autorizar el tratamiento de datos' });
    if (versionTerminos !== TERMINOS_VERSION || versionPolitica !== POLITICA_VERSION)
        return res.status(400).json({ error: 'Versión de documentos desactualizada' });
...
app.post('/api/auth/login', async (req, res) => {
    ...
    const emailNorm = email?.trim().toLowerCase();
    if (emailNorm !== ADMIN_EMAIL)
        return res.status(401).json({ error: 'Acceso solo para administradores con contraseña' });
...
app.put('/api/notifications/:id/read', auth, async (req, res) => {
    await getDb().collection('notificaciones').updateOne({ _id: new ObjectId(req.params.id), usuario_id: new ObjectId(req.user.id) }, { $set: { leida: true } });
...
app.get('/api/products', async (_req, res) => {
    const docs = await getDb().collection('productos')
        .find({
        activo: { $ne: false },
        descripcion: { $nin: ['NABHAN INVESMENT SAS - MEGUIARS', 'RED LLANTA S.A', 'FERRO AUTOMOTRIZ S.A.S'] },
        nombre: {
...
app.get('/api/appointments/available', async (req, res) => {
    ...
    if (!fecha || !servicioId || !isValidObjectId(servicioId))
        return res.json(HORARIOS.map(h => ({ value: h, label: HORARIO_LABELS[h] || h })));
...
app.get('/api/appointments/calendar', async (req, res) => {
    ...
    if (!servicioId || !year || !month || !isValidObjectId(servicioId))
...
app.put('/api/appointments/:id/cancel', auth, async (req, res) => {
    ...
    if (!cita || String(cita.usuario_id) !== req.user.id)
...
const analyticsCache = { data: null, loadedAt: 0 };
app.get('/api/admin/dashboard/analytics', auth, adminRequired, async (_req, res) => {
    if (analyticsCache.data && Date.now() - analyticsCache.loadedAt < 60000)
...
app.get('/api/admin/dashboard/powerbi', auth, adminRequired, async (_req, res) => {
    ...
    const mType = (v) => {
        if (v === null || v === undefined)
            return 'null';
        if (typeof v === 'number')
            return Number.isInteger(v) ? 'Int64.Type' : 'number';
        if (typeof v === 'boolean')
            return 'Logical.Type';
        return 'text';
    };
    const buildTableM = (name, rows, cols) => {
        if (rows.length === 0)
            return `#table(${JSON.stringify(cols)}, {})`;
    ...
    const { ZipArchive: Za } = await import('archiver');
...
app.post('/api/admin/import', auth, adminRequired, upload.single('archivo'), async (req, res) => {
    ...
    if (ext === 'csv') {
        ...
        const wb = await new ExcelJS.Workbook().xlsx.load(req.file.buffer);
    ...
    if (tipo === 'combinado') {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            if (!producto) {
                errores.push(`Fila ${nro}: falta PRODUCTO`);
                continue;
            }
            ...
            try {
                ...
                if (tipo === 'servicios') {
                    const doc = {
                        ...
                        items: row.items?.trim() ? row.items.split(',').map((s) => s.trim()) : [],
