...
function findImage(name, map, defaultImg) {
...
function categorizeProduct(name) {
    ...
    for (const w of words) {
        if (isFoodWord(w))
            foodScore += 2;
        else if (isDrinkWord(w))
            drinkScore += 2;
        else if (isCleaningWord(w))
            cleaningScore += 2;
        else if (w === 'GATORLIT' || w === 'HIDRA')
            foodScore += 2;
    }
    ...
    if (cleaningScore > drinkScore && cleaningScore > foodScore && cleaningScore > 0)
        return IMG.CLEANING;
    if (drinkScore > foodScore && drinkScore > 0)
        return IMG.JUICE;
    if (foodScore > 0)
        return IMG.FOOD;
    return IMG.PARTS;
