...
export async function notificarCitaAgendada(usuarioId, email, servicioNombre, fecha, horario, reference) {
    ...
    await Promise.all([
        ...
        enviarNotificacionGeneral({
            ...
            if (!sent)
                console.log(`[NOTIF] Email de cita encolado como pendiente: ${email}`);
        })
    ]);
}
export async function notificarBienvenidaCliente(usuarioId, email) {
    ...
    await Promise.all([
        ...
        enviarNotificacionGeneral({
            ...
            if (!sent)
                console.log(`[NOTIF] Email de bienvenida encolado como pendiente: ${email}`);
        })
    ]);
}
