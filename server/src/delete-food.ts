...
function isFoodProduct(name: string): boolean {
  ...
  if (words.some((w: string) => AUTO_WORDS.has(w))) return false;
  if (name.toUpperCase().includes('PARABRISA') || name.toUpperCase().includes('PARABRISAS')) return false;
  if (name.toUpperCase().includes('EXTINTOR')) return false;
  if (name.toUpperCase().includes('LATONERIA')) return false;

  // Check for food/drink words (word-level only, no substring matching)
  for (const w of words) {
    if (FOOD_WORDS.has(w) || DRINK_WORDS.has(w) || COFFEE_WORDS.has(w)) return true;
  }
...
async function main() {
  ...
  const svcToDelete = allSvcs.filter(s => {
    ...
    const hasAuto = tokens.some((t: string) => autoKeywords.some((k: string) => t.startsWith(k) || k.startsWith(t)));
