...
function findMatch(normalized: string, lookup: Map<string, any>): any {
  // 1. Exact match
  ...
  for (const [key, doc] of lookup) {
    ...
    for (const t of tokens) {
      if (keyTokens.some((kt: string) => kt === t || kt.startsWith(t) || t.startsWith(kt))) {
        matches++;
      }
    }
    // Also check inverted
    for (const kt of keyTokens) {
      if (tokens.some((t: string) => t === kt || t.startsWith(kt) || kt.startsWith(t))) {
...
async function main() {
  ...
  const svcLookup = new Map<string, any>();
  ...
  // ── Read Excel ──
  const excelProds = new Map<string, { proveedor: string; precio: number; stock: number; clase: string }>();
  const excelSvcs = new Map<string, { proveedor: string; precio: number; clase: string }>();
  const foodSkipped: string[] = [];
  ...
  for (const ws of wb.worksheets) {
    ...
    ws.eachRow((row, r) => {
      ...
      if (!nombre || isFoodItem(nombre)) {
        if (nombre && isFoodItem(nombre)) foodSkipped.push(nombre);
        return;
      }
  ...
  const excelProdNorms = new Set<string>();
  for (const [name] of excelProds) excelProdNorms.add(normalize(name));
  ...
  for (const [nombre, data] of excelSvcs) {
    ...
    if (match) {
      ...
      await db.collection('servicios').updateOne(
        { _id: match._id, precio_moto: { $exists: false } },
        { $set: { precio_moto: Math.round(precioAuto * 0.8) } }
      );
  ...
  const excelSvcNorms = new Set<string>();
  for (const [name] of excelSvcs) excelSvcNorms.add(normalize(name));
