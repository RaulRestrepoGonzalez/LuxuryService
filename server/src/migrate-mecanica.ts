...
function isMechanical(name: string): boolean {
  ...
  for (const kw of MECHANICAL) {
    if (upper.startsWith(kw) || upper.includes(kw)) {
      ...
      for (const ex of EXCLUDE) {
        if (upper.startsWith(ex) || upper.includes(ex)) return false;
      }
      return true;
    }
  }
...
async function migrate() {
  ...
  const toDelete = all
    .filter(d => isMechanical(d.nombre as string))
    .filter(d => (d.nombre as string) !== 'Pintura y Acabados' && (d.nombre as string) !== 'Latonería y Carrocería');
  ...
  if (toDelete.length === 0) {
    ...
    for (const d of toDelete) console.log(`  · ${d.nombre} (${d.categoria})`);
